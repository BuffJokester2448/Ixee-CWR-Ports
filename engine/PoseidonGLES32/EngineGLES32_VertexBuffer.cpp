...
class VertexBufferGLES32 : public VertexBuffer
{
  ...
  public:
    VertexBufferGLES32() = default;
    ~VertexBufferGLES32() override;
...
VertexBufferGLES32::~VertexBufferGLES32()
{
    if (_vao)
        GLES32Bind::OnVaoDeleted(_vao);
...
void VertexBufferGLES32::SetupVertexAttribs()
{
...
void VertexBufferGLES32::CopyVertices(const Shape& src)
{
    if (_vertexCount <= 0)
        return;
...
bool VertexBufferGLES32::Init(const Shape& src, VBType type)
{
    if (src.NVertex() <= 0)
    {
        LOG_DEBUG(Graphics, "GLES32: Empty vertices.");
        return false;
    }
...
void VertexBufferGLES32::Update(const Shape& src, bool dynamic)
{
...
VertexBuffer* EngineGLES32::CreateVertexBuffer(const Shape& src, VBType type)
{
    auto* buf = new VertexBufferGLES32;
...
int EngineGLES32::CompareBuffers(const Shape&, const Shape&)
{
    return 0;
}

void EngineGLES32::DrawSectionTL(const Shape& sMesh, int beg, int end)
{
    auto* buf = static_cast<VertexBufferGLES32*>(sMesh.GetVertexBuffer());
    ...
    LOG_DEBUG(Graphics, "GLES32: DrawSectionTL #{} indices={} worldXYZ=({},{},{})", _dbgMeshDrawCalls, indexCount,
...
void EngineGLES32::EmitDraw(const Poseidon::render::frame::Draw& d)
{
    if (!_glContext)
        return;
    ...
    if (d.textures[1].id != 0)
        GLES32Bind::Tex2D(1, d.textures[1].id);
    if (d.textures[0].id != 0)
        GLES32Bind::Tex2D(0, d.textures[0].id);
    else
...
void EngineGLES32::Clear(bool clearZ, bool clear, PackedColor color)
{
    if (!_glContext)
        return;
    GLbitfield mask = 0;
...
void EngineGLES32::ReportGRAM(const char*) {}

AbstractTextBank* EngineGLES32::TextBank()
{
    return _textBank;
}

void EngineGLES32::SetRenderScale(float scale)
{
    if (scale < 1.0f)
        scale = 1.0f;
    if (scale > 2.0f)
...
void EngineGLES32::SetMsaaSamples(int samples)
{
...
void EngineGLES32::RenderTargetSize(int& w, int& h) const
{
    if (SSAAActive())
    {
        w = _ssaaW;
...
void EngineGLES32::ApplyPendingRenderScale()
{
    int winW = _w, winH = _h;
    if (_sdlWindow)
    ...
    {
        LOG_WARN(Graphics,
                 "GLES32: frame target {}x{} incomplete (0x{:x}/0x{:x}) — rendering directly to the window (no MSAA/SSAA)",
    ...
    LOG_INFO(Graphics, "GLES32: frame target {}x{} ({}x MSAA, render scale {}), window {}x{}", sw, sh,
             static_cast<int>(samples), want, winW, winH);
}

void EngineGLES32::ResolveSSAAToDefault()
{
    if (!SSAAActive())
        return;
    int winW = _w, winH = _h;
...
void EngineGLES32::BindFrameRenderTarget()
{
...
void EngineGLES32::WorkToBack() {}

void EngineGLES32::CaptureScreenshotIfPending()
{
...
int EngineGLES32::SampleBackBufferNonBlack()
{
    if (!_glContext)
        return -1;
...
bool EngineGLES32::SamplePixel(int x, int y, uint8_t* outRGB)
{
