...
void EngineGLES32::DrawDecal(Vector3Par screen, float rhw, float sizeX, float sizeY, PackedColor color,
...
void EngineGLES32::DrawPolygon(const VertexIndex* ii, int n)
{
    QueueFan(ii, n);
}

void EngineGLES32::DrawSection(const FaceArray& face, Offset beg, Offset end)
{
    if (_resetNeeded)
    {
        return;
...
void EngineGLES32::DrawPoints(const TLVertex* vs, int nVertex)
{
    if (_resetNeeded)
    {
        return;
    ...
    for (int i = 0; i < nVertex; i++)
    {
        ...
        // 2D/HUD path (EngineGLES32_2D Draw2D).
...
void EngineGLES32::DrawPoints(int beg, int end)
{
    // now we are drawing quads directly
...
bool EngineGLES32::CanGrass() const
{
    return false;
}
