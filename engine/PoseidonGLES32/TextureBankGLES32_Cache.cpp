...
void TextBankGLES32::StartFrame()
{
    InitDetailTextures();
    CheckTextureMemory();
    _thisFrameCopied = 0;
...
void TextBankGLES32::FinishFrame()
{
    for (HMipCacheGLES32* tc = _thisFramePartialUsed.Last(); tc; tc = _thisFramePartialUsed.Prev(tc))
    {
        TextureGLES32* tex = tc->texture;
        tex->_levelNeededLastFrame = tex->_levelNeededThisFrame;
        tex->_levelNeededThisFrame = tex->_nMipmaps;
        tex->ResetMipmap();
    }
    for (HMipCacheGLES32* tc = _thisFrameWholeUsed.Last(); tc; tc = _thisFrameWholeUsed.Prev(tc))
    {
        TextureGLES32* tex = tc->texture;
        tex->_levelNeededLastFrame = tex->_levelNeededThisFrame;
        tex->_levelNeededThisFrame = tex->_nMipmaps;
    ...
    for (HMipCacheGLES32* tc = _lastFramePartialUsed.Last(); tc; tc = _lastFramePartialUsed.Prev(tc))
    {
        TextureGLES32* tex = tc->texture;
        tex->_levelNeededLastFrame = tex->_levelNeededThisFrame;
        tex->_levelNeededThisFrame = tex->_nMipmaps;
    ...
    for (HMipCacheGLES32* tc = _lastFrameWholeUsed.Last(); tc; tc = _lastFrameWholeUsed.Prev(tc))
    {
        TextureGLES32* tex = tc->texture;
        tex->_levelNeededLastFrame = tex->_levelNeededThisFrame;
        tex->_levelNeededThisFrame = tex->_nMipmaps;
...
DEFINE_FAST_ALLOCATOR(HMipCacheGLES32);

const int MaxAllocationsPerFrame = 8;
const int MaxCopyPerFrame = 32768;

MipInfo TextBankGLES32::UseMipmap(Texture* absTexture, int level, int top)
{
    if (!absTexture)
        return MipInfo(nullptr, 0);

    TextureGLES32* texture = static_cast<TextureGLES32*>(absTexture);
    ...
    if (texture->_levelLoaded > level)
    {
        ...
        for (;;)
        {
            ...
            if (ret >= 0)
                break;
            LOG_DEBUG(Graphics, "GLES32: Out of VID: Try next level");
            level++;
...
void TextBankGLES32::InitDetailTextures()
{
    if (_detail)
        return;
...
void TextBankGLES32::FlushTextures()
{
...
void TextBankGLES32::ForceReloadAll()
{
    ...
    LOG_INFO(Graphics, "TextBankGLES32::ForceReloadAll: dropped {} textures, kept {} dynamic", dropped, skipped);
}

void TextBankGLES32::FlushBank(QFBank* bank)
{
    for (int i = 0; i < _texture.Size(); i++)
    {
        TextureGLES32* tex = _texture[i];
...
static int CompareTextureFileOrderGL33(const LLink<TextureGLES32>* tl1, const LLink<TextureGLES32>* tl2)
{
    TextureGLES32* t1 = *tl1;
    TextureGLES32* t2 = *tl2;
...
void TextBankGLES32::Preload()
{
    ...
    LOG_DEBUG(Graphics, "GLES32: Preload {} textures - {} ms", _texture.Size(), end - start);
}

int TextBankGLES32::FreeTextureMemory()
{
...
void TextBankGLES32::CheckTextureMemory()
{
