...
void EngineGLES32::DestroySamplerStates()
{
...
void EngineGLES32::ApplyBlendMode(BlendMode mode)
{
    if (!_glContext)
        return;
...
void EngineGLES32::ApplyDepthMode(DepthMode mode)
{
    if (!_glContext)
        return;
...
void EngineGLES32::SetAlphaTest(bool enable, DWORD ref, bool alphaToCoverage)
{
...
void EngineGLES32::BeginDebugGroup(const char* name)
{
...
void EngineGLES32::EndDebugGroup()
{
    if (glPopDebugGroup)
    {
        glPopDebugGroup();
...
void EngineGLES32::SwitchPassDebugGroup(const char* name)
{
...
void EngineGLES32::ClosePassDebugGroup()
{
    if (_passDebugGroupOpen)
    {
        EndDebugGroup();
...
bool EngineGLES32::GetGLViewport(int outRect[4]) const
{
    if (!_glContext)
        return false;
    GLint vp[4] = {0, 0, 0, 0};
...
void EngineGLES32::DoStencilExclusion(bool enable, bool)
{
...
void EngineGLES32::ChangeClipPlanes() {}

void EngineGLES32::PrepareDetailTex(bool water, bool grass)
{
    LOG_DEBUG(Graphics, "GLES32: PrepareDetailTex water={} grass={} multiTex={}", water, grass, IsMultitexturing());
    if (!IsMultitexturing())
    {
...
void EngineGLES32::PrepareSingleTexModulateA()
{
...
void EngineGLES32::PrepareSingleTexDiffuseA()
{
    EnableDetailTexGen(IsIn3DPass() ? TGNone : TGFixed, true);
    SelectPixelShader(PSNormal);
    SetMultiTexturing(SingleTex);
...
void EngineGLES32::DoEnableDetailTexGen(TexGenMode mode, bool)
{
    _texGenMode = mode;
    UploadVSTexGenConstants(mode);
}

void EngineGLES32::Init3DState()
{
    if (!_glContext)
        return;
    ...
    // _canZBias) are static constexpr in EngineGLES32.hpp — nothing to set here.
...
void EngineGLES32::ApplyWorldViewport()
{
...
void EngineGLES32::EndWorldViewport()
{
...
void EngineGLES32::SetTexture(const TextureGLES32* tex, const Poseidon::render::LegacySpec& spec)
{
...
void EngineGLES32::SetMultiTexturing(VFormatSet format)
{
    ...
    switch (format)
    {
        ...
        case DetailTex:
        {
            TextureGLES32* detail = _textBank ? _textBank->GetDetailTexture() : nullptr;
            if (_textBank && detail)
                _textBank->UseMipmap(detail, 0, 0);
            unsigned int dh = detail ? detail->GetHandle() : 0;
            LOG_DEBUG(Graphics, "GLES32: SetMultiTex DetailTex handle={} detail={}", dh, (void*)detail);
            boundHandle = dh ? dh : _fallbackWhiteTex;
            break;
        }
        case GrassTex:
        {
            TextureGLES32* grass = _textBank ? _textBank->GetGrassTexture() : nullptr;
            if (_textBank && grass)
        ...
        case SpecularTex:
        {
            TextureGLES32* spec = _textBank ? _textBank->GetSpecularTexture() : nullptr;
            if (_textBank && spec)
    ...
    GLES32Bind::Tex2D(1, boundHandle);
