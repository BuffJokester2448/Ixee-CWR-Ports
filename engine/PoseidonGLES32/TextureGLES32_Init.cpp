...
int MipmapSizeGLES32(PacFormat format, int w, int h)
{
    switch (format)
    {
        case PacDXT1:
...
static PacFormat DstFormat(PacFormat srcFormat, int dxt)
{
    switch (srcFormat)
    {
        ...
        case PacP8:
            return PacARGB1555;
        case PacAI88:
            if (static_cast<EngineGLES32*>(GEngine)->Can88())
                return srcFormat;
            if (static_cast<EngineGLES32*>(GEngine)->Can8888())
                return PacARGB8888;
            return PacARGB4444;
        case PacDXT1:
...
void TextureGLES32::SetMipmapRange(int min, int max)
{
    if (min < 0)
        min = 0;
    if (max > _nMipmaps - 1)
...
int TextureGLES32::Init(const char* name)
{
    SetName(name);
...
void TextureGLES32::PreloadHeaders()
{
    RString resolved = Poseidon::Graphics::ResolveLooseTexturePath(Name());
    ITextureSourceFactory* factory = SelectTextureSourceFactory(resolved);
    if (!factory)
...
void TextureGLES32::DoLoadHeaders()
{
    ...
    if (_src)
    {
        ...
        int dxt = static_cast<EngineGLES32*>(GEngine)->DXTSupport();
...
void TextureGLES32::LoadHeaders()
{
    if (_initialized)
        return;
    DoLoadHeaders();
...
Color TextureGLES32::GetPixel(int level, float u, float v) const
{
    LoadHeadersNV();
...
Poseidon::AlphaStats::Kind TextureGLES32::GetAlphaClass()
{
    if (_alphaClass >= 0)
...
DEFINE_FAST_ALLOCATOR(TextureGLES32);

TextureGLES32::TextureGLES32()
...
void TextureGLES32::SetMaxSize(int size)
{
    LoadHeadersNV();
    if (size >= _maxSize)
        return;
...
void TextureGLES32::SetMultitexturing(int type)
{
    _useDetail = (type != 0);
}

bool TextureGLES32::VerifyChecksum(const MipInfo&) const
{
    return true;
}

void TextureGLES32::ASetNMipmaps(int n)
{
    LoadHeadersNV();
    if (n > _nMipmaps)
    {
...
int SurfaceInfoGLES32::_nextId = 0;

int SurfaceInfoGLES32::CalculateSize(const TextureDescGLES32& desc, PacFormat format, int totalSize)
{
    if (totalSize >= 0)
        return totalSize;
    ...
    {
        size += MipmapSizeGLES32(format, w, h);
        w = (w > 1) ? w / 2 : 1;
        h = (h > 1) ? h / 2 : 1;
    }
    return size;
...
int SurfaceInfoGLES32::CreateSurface(const TextureDescGLES32& desc, PacFormat format, int totalSize)
{
    _w = desc.w;
    _h = desc.h;
    ...
    {
        LOG_ERROR(Graphics, "GLES32: glTexStorage2D FAILED err=0x{:04X} tex={} {}x{} mips={} fmt=0x{:04X}", allocErr,
...
void SurfaceInfoGLES32::Free(bool lastRef, int)
{
    if (lastRef && _texture)
    {
        GLES32Bind::OnTexDeleted(_texture);
...
bool TextureGLES32::InitFromRGBA(int w, int h, const void* rgba, uint32_t size, bool mipmap)
{
    if (!rgba)
        return false;
...
void TextureGLES32::UpdateRGBA(const void* rgba, uint32_t size)
{
    if (!_surface.GetTexture() || !rgba)
        return;

    GLES32Bind::Tex2D(EngineGLES32::kUploadUnit - GL_TEXTURE0, _surface.GetTexture());
